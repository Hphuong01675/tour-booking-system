...
const ChatMessageContent = ({ content, isGuide, onPreviewImage }) => {
  const parsed = parseChatContent(content);
  ...
  if (parsed.type === 'cccd_review') {
    ...
    return (
      <div className={`w-72 max-w-full space-y-3 ${isGuide ? 'text-on-primary' : 'text-on-surface'}`}>
        <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {images.map((item) => (
            <button
              ...
              className="text-left"
            >
              <img src={item.url} alt={`CCCD ${item.label}`} className="w-full h-24 object-cover rounded-lg bg-black/5 cursor-zoom-in" />
...
const GuideChatPage = () => {
  ...
  // Khá»Ÿi táº¡o Socket vĂ  Fetch danh sĂ¡ch há»™i thoáº¡i
  useEffect(() => {
  ...
  const handleSendMedia = async (file) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || err.response?.data?.error || 'KhĂ´ng thá»ƒ gá»­i tá»‡p. Vui lĂ²ng thá»­ láº¡i.');
    } finally {
  ...
  return (
      <>
        <main className="flex-grow flex overflow-hidden h-full w-full min-w-0">
          <aside className="w-full max-w-[400px] min-h-0 shrink-0 border-r border-outline-variant bg-surface flex flex-col">
            <div className="p-lg space-y-md border-b border-outline-variant">
              <div className="flex items-center justify-between">
                <h2 className="font-headline-sm text-headline-sm text-on-surface">TÆ° váº¥n khĂ¡ch hĂ ng</h2>
                <span className="bg-primary-container text-on-primary-fixed text-label-md px-sm py-xs rounded-full font-bold">
                {conversations.filter(c => c.status === 'waiting').length} má»›i
              </span>
              </div>
              ...
              <div className="flex bg-surface-container rounded-xl p-xs">
                ...
                >
                  Chá» tiáº¿p nháº­n ({conversations.filter(c => c.status === 'waiting').length})
                </button>
                <button
              ...
              <div className="relative">
                <input
                    ...
                    placeholder="TĂ¬m kiáº¿m khĂ¡ch hĂ ng..."
                />
            ...
            <div className="flex-1 min-h-0 overflow-y-auto chat-scrollbar p-md space-y-sm">
              {visibleConversations.length > 0 ? (
                  ...
                  <div className="p-xl text-center text-outline italic">
                    KhĂ´ng cĂ³ cuá»™c há»™i thoáº¡i nĂ o.
                  </div>
              )}
            </div>
          </aside>
          ...
          <section className="flex-1 min-w-0 min-h-0 bg-surface-container-lowest flex flex-col relative overflow-hidden">
            {activeConv ? (
                  ...
                  <header className="h-[72px] px-lg flex items-center justify-between bg-surface-container-lowest/80 backdrop-blur-md z-10 border-b border-outline-variant">
                    <div className="flex items-center gap-md">
                      ...
                      <div>
                        ...
                        <p className="text-label-sm text-primary">Äang trá»±c tuyáº¿n</p>
                      </div>
                    </div>
                          ...
                          >
                            ...
                            <div className={`space-y-xs min-w-0 ${isGuide ? 'items-end flex flex-col' : ''}`}>
                              <div className={`p-md rounded-2xl text-body-md ${
                                ...
                                <ChatMessageContent content={msg.content} isGuide={isGuide} onPreviewImage={setPreviewImage} />
                              </div>
                    ...
                    ></textarea>
                            ...
                            <div className="flex gap-xs items-center">
                              ...
                              {QUICK_EMOJIS.map((emoji) => (
                                  <button
                                      ...
                                      title={`Gá»­i biá»ƒu tÆ°á»£ng ${emoji}`}
                                  >
                                    {emoji}
                                  </button>
                              ))}
                        ...
                        <div className="p-lg bg-surface-container rounded-2xl border border-dashed border-outline-variant flex flex-col items-center justify-center">
                          ...
                          <p className="font-label-md text-on-surface-variant text-center">
                            Báº¡n pháº£i Tiáº¿p nháº­n cuá»™c há»™i thoáº¡i nĂ y trÆ°á»›c khi gá»­i tin nháº¯n tÆ° váº¥n.
                          </p>
                          <button
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center p-xl text-center">
                  ...
                  <h3 className="font-headline-sm text-on-surface mb-xs">ChĂ o má»«ng Ä‘áº¿n vá»›i Trá»±c Tá»•ng ÄĂ i TÆ° Váº¥n</h3>
                  <p
                  ...
                  >
                    HĂ£y chá»n má»™t cuá»™c há»™i thoáº¡i Ä‘á»ƒ báº¯t Ä‘áº§u há»— trá»£.
                  </p>
                </div>
            )}
          </section>
          ...
          {activeConv && (
              <aside className="hidden xl:flex w-[320px] bg-surface flex-col border-l border-outline-variant">
                <div className="p-lg border-b border-outline-variant">
                  <h4 className="font-label-md text-outline uppercase tracking-wider mb-md">ThĂ´ng tin khĂ¡ch hĂ ng</h4>
                  <div className="space-y-md">
                    <div className="flex justify-between items-center">
                      <span className="text-body-sm text-outline">Há» tĂªn</span>
                      <span className="text-body-sm font-semibold text-on-surface">
                    {activeConv.customer ? activeConv.customer.fullName : activeConv.guestName}
                  </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-body-sm text-outline">Sá»‘ ÄT</span>
                      <span className="text-body-sm font-semibold text-on-surface">
                    {activeConv.customer?.phone || 'ChÆ°a cáº­p nháº­t'}
                  </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-lg flex-1 overflow-y-auto chat-scrollbar">
                  <h4 className="font-label-md text-outline uppercase tracking-wider mb-md">Ghi chĂº há»— trá»£</h4>
                  <textarea
                      ...
                      placeholder="Nháº­p ghi chĂº nhanh vá» khĂ¡ch hĂ ng nĂ y..."
                  />
                </div>
              </aside>
          )}
