...
const OTPVerificationPage = () => {
  ...
  const submitOTP = async (otpToSubmit) => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.error || 'Xác thực OTP thất bại. Vui lòng thử lại.';
      setError(errorMsg);
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.error || 'Gửi lại mã OTP thất bại. Vui lòng thử lại.';
      setError(errorMsg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-on-background font-body-md">
      ...
      <main className="flex-grow flex items-center justify-center pt-16 px-4">
        <div className="max-w-md w-full py-xl">
          {/* OTP Container Card */}
          <div className="bg-surface-container-lowest rounded-xl shadow-lg p-lg md:p-xl flex flex-col gap-lg border border-outline-variant">
            {/* Icon Section */}
            <div className="flex flex-col items-center text-center gap-sm">
              <div className="w-16 h-16 bg-primary-fixed rounded-full flex items-center justify-center mb-sm">
            ...
            {/* Messages */}
            {error && (
              <div className="bg-error-container/20 border border-error text-error-container text-label-sm p-md rounded-lg">
                {error}
              </div>
            )}
            {success && (
              <div className="bg-green-100/20 border border-green-500 text-green-700 text-label-sm p-md rounded-lg">
                {success}
              </div>
            )}
            ...
            <div className="flex flex-col gap-md">
              ...
              {/* Countdown */}
              <div className="flex items-center justify-center gap-xs text-body-sm">
            ...
            <div className="flex flex-col gap-md">
              <button
                ...
                className="w-full bg-secondary-container hover:bg-secondary disabled:bg-gray-400 text-white py-md rounded-lg font-label-md transition-all active:scale-[0.98] shadow-md"
              >
          ...
          {/* Decorative Image */}
          <div className="mt-xl rounded-xl overflow-hidden relative h-48 md:h-64 shadow-md group">
            <img
              alt="Security Travel"
      ...
      <footer className="bg-surface-container-low dark:bg-surface-container-lowest border-t border-outline-variant">
        <div className="flex flex-col md:flex-row justify-between items-center px-margin-mobile md:px-margin-desktop py-lg w-full gap-md">
