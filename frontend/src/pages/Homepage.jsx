...
const Homepage = () => {
    ...
    // Realtime Guest Chat Logic
    const [conversationId, setConversationId] = useState(null);
    const socketRef = useRef(null);

    useEffect(() => {
        let sessionKey = localStorage.getItem("guest_session_key");
        if (!sessionKey) {
            sessionKey = 'guest_' + Math.random().toString(36).substr(2, 9) + Date.now().toString(36);
        ...
        const initChat = async () => {
            try {
                ...
                const formattedMessages = messages.map(msg => ({
                    id: msg.id,
                    sender: msg.senderType === 'guest' ? 'user' : 'bot',
                    text: msg.content,
                    time: new Date(msg.sentAt).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" })
                }));
                setChatMessages(formattedMessages);

                if (!socketRef.current) {
                    socketRef.current = io(import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000');
                    socketRef.current.emit("join_room", conversation.id);
                    ...
                    socketRef.current.on("receive_message", (newMsg) => {
                        setChatMessages(prev => {
                            ...
                            return [...prev, {
                                id: newMsg.id,
                                sender: newMsg.senderType === 'guest' ? 'user' : 'bot',
                                text: newMsg.content,
                                time: new Date(newMsg.sentAt).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" })
                            }];
                        });
                    });
                }
        ...
        return () => {
            // We keep socket alive while component is mounted to receive messages if closed and reopened
        };
    }, [isChatOpen, conversationId]);

    const handleSendChatMessage = (e) => {
        if (e) e.preventDefault();
        if (!currentChatMessage.trim() || !conversationId || !socketRef.current) return;

        const content = currentChatMessage.trim();
