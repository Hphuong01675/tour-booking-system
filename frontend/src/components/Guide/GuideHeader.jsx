...
const GuideHeader = ({ currentUser, onLogoutClick }) => {
  ...
  const activeClass = "font-body-md text-body-md text-primary dark:text-inverse-primary border-b-2 border-primary dark:border-inverse-primary pb-1 font-semibold transition-all duration-200";
  const inactiveClass = "font-body-md text-body-md text-on-surface-variant dark:text-outline-variant hover:text-primary dark:hover:text-inverse-primary transition-colors";

  return (
      <header className="bg-surface-container-lowest dark:bg-inverse-surface shadow-sm sticky top-0 z-50 flex justify-between items-center w-full px-margin-desktop h-20">
        ...
        <div className="flex items-center gap-md">
          ...
          <div className="flex items-center gap-sm relative" ref={dropdownRef}>
            ...
            <div className={`absolute right-0 top-full mt-2 w-48 bg-surface-container-lowest dark:bg-inverse-surface border border-outline-variant/30 rounded-lg shadow-lg transition-all duration-200 z-50 overflow-hidden ${isDropdownOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
