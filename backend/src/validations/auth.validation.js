...
class AuthValidation {
  ...
  isValidVietnameseName(fullName) {
    const vietnameseNameRegex = /^[\p{L}\s\-.,0-9]+$/u;
  ...
  validateForgotPassword = (req, res, next) => {
    ...
    if (!email) {
      return res.status(400).json({ error: 'Email là bắt buộc.', code: 'MISSING_EMAIL' });
    }
  ...
  validateResetPassword = (req, res, next) => {
    ...
    if (!newPassword) {
      return res.status(400).json({ error: 'Mật khẩu mới là bắt buộc.', code: 'MISSING_PASSWORD' });
    }
