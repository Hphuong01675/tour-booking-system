...
String waitOtpFromBackendLog(String email, String logPath, int timeoutSeconds) {
    ...
    throw new Exception("KhĂ´ng tĂ¬m tháº¥y OTP trong backend.log cho email: " + email + ". Log path: " + logPath)
}
...
println(">>> OTP láº¥y tá»« backend.log lĂ : [${otpCode}]")
WebUI.takeScreenshot()

if (!otpCode || otpCode.length() != 4) {
    throw new Exception("OTP khĂ´ng há»£p lá»‡: " + otpCode)
}
...
if (WebUI.verifyElementPresent(findTestObject('DangKy/input_OTP'), 3, com.kms.katalon.core.model.FailureHandling.OPTIONAL)) {
    ...
    // Fallback náº¿u OTP lĂ  4 Ă´ input riĂªng
    WebUI.executeJavaScript("""
